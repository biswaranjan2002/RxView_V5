...
public class PrintAngleDraw implements PrintInterfaceDrawTools{
    ...
    public PrintDegreeLabel degreeLabel;
    ...
    private PrintCanvasMainChange printCanvasMainChange=null;

    public PrintAngleDraw() {
    }

    public PrintAngleDraw(Canvas theCanvas, Point2D point, PrintCanvasMainChange printCanvasMainChange){
        this.canvas = theCanvas;
        this.clickCount = 0;
        this.printCanvasMainChange = printCanvasMainChange;
        printCanvasMainChange.isAngleDrawComplete=false;
    ...
    public PrintDegreeLabel getDegreeLabel() {
        return degreeLabel;
    }

    public void setDegreeLabel(PrintDegreeLabel degreeLabel) {
    ...
    public void handleMousePressed(MouseEvent event) {
        ...
        for (Ellipse ellipse : ellipses) {
            if (printCanvasMainChange.isAngleDrawComplete && isPointInEllipse(clickPoint, ellipse)) {
    ...
    public void showDegree(Ellipse e1, Ellipse e2, Ellipse e3) {
        ...
        printCanvasMainChange.removeDegreeLabel(degreeLabel);

        degreeLabel = new PrintDegreeLabel(angleDeg, px, py, Color.rgb(255, 235, 42));

        printCanvasMainChange.addDegreeLabel(degreeLabel);
    }
    ...
    private void handleClick(Point2D clickPoint) {
        if(clickCount < 3){
            ...
            if(clickCount == 0){
                ...
                drawLine(ellipses.get(clickCount - 1), ellipses.get(clickCount));
                printCanvasMainChange.isAngleDrawComplete = true;
            }
            clickCount++;
        }
    }
    ...
    private void drawLine(Ellipse start, Ellipse end) {
        ...
        line.setStrokeWidth(printCanvasMainChange.printDynamicCanvasElementsResize.getLineWidth());
        lines.add(line);
    ...
    public void redraw(PrintCanvasMainChange printCanvasMainChange) {
        ...
        if(ellipses.size() == 3){
            ...
            gc.strokeText(String.format("%.2f°", angleDeg), px, py+ 2* printCanvasMainChange.printDynamicCanvasElementsResize.getLabelFontSize());


            gc.setFill(Color.valueOf("#66FF00"));
            gc.setFont(Font.font("Century Gothic", FontWeight.NORMAL, 2 * printCanvasMainChange.printDynamicCanvasElementsResize.getLabelFontSize()));
            gc.fillText(String.format("%.2f°", angleDeg), px, py+ 2*printCanvasMainChange.printDynamicCanvasElementsResize.getLabelFontSize());
        }
        else
        {
            for (Line line : lines) {
    ...
    public void removeAll() {
        ...
        ellipses.clear();
        printCanvasMainChange.removeAllLines(lines);
        lines.clear();
        printCanvasMainChange.removeDegreeLabel(degreeLabel);
        redraw(printCanvasMainChange);
        printCanvasMainChange.isAngleDrawComplete = true;
